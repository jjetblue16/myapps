...
void gamePadJoystick(ControllerPtr ctl) {
    if (abs(ctl->axisY()) > 35) {
        acceleration(translateJoystick(ctl->axisY()));
    }
    else {
        acceleration(0);
    }
    if(abs(ctl->axisRX())>35) {
        steering(translateJoystick(ctl->axisRX()));
    }
    else    {
        steering(0);
    }
